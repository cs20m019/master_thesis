...
def run_art_evasion_pgd(SKmodel, X_test, y_test, verbose):
    ...
    attack = ProjectedGradientDescentNumpy(estimator=estimator, eps=input_variation, max_iter=maximum_iterations, batch_size=b_size, verbose=verbose)
...
def run_art_poisoning_svm(SKmodel, model_name, X_train, X_test, y_train, y_test, verbose):
    ...
    # classifier = SKmodel
    # art_classifier = ScikitlearnLinearSVC(model=SKmodel)
    ...
    attack_index = 6 # datapoint from which the algorith starts, this is arbiraty
    ...
    # Convert to array
    y_pred_poisoned = np.array(y_pred)
